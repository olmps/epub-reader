...
class EpubParser {
  ...
  static String get _resourcesDirectoryPath => _rootContentFile.filePath
      .substring(0, _rootContentFile.filePath.lastIndexOf('/'));
  ...
  static _extractEpubContent(String path, OnProgress onProgress) async {
    ...
    _extractedEpubDirectory =
        _extractedEpubDirectory.createTempSync("${epubFileName}_unzipped");
  ...
  static _loadContentFile() async {
    try {
      ...
      final manifestPath =
          "${_extractedEpubDirectory.path}/$manifestContainerPath";
      final manifestXml = XMLEntity(manifestPath);
      await manifestXml.load();

      final rootfileTagAttributes =
          manifestXml["rootfiles"]["rootfile"].attributes();
  ...
  static Future<String> _chapterContent(String path) async {
    ...
    for (var css in allCss) {
      ...
      chapterContent = chapterContent.replaceAllMapped("<head>", (match) => "${match.group(0)} <style>$cssContent</style>");
    ...
    for (var js in allJs) {
      ...
      chapterContent = chapterContent.replaceAllMapped("<head>", (match) => "${match.group(0)} <script>$jsContent</script>");
  ...
  static Future<EpubMetadata> _readMetadata() async {
    ...
    // Cover Page
    final coverMeta =
        allMetas.firstWhere((meta) => meta.attributes()["name"] == "cover");
    ...
    final highlightClassMeta = allMetas.firstWhere((meta) {
      return meta.attributes().containsKey("property") && 
            meta.attributes()["property"] == "media:active-class";
    }, orElse: () => null);
