...
import 'package:epub_reader/audio_player.dart';
import 'package:epub_reader/models/chapter.dart';
import 'package:flutter/material.dart';
import 'package:webview_flutter/webview_flutter.dart';
import 'package:epub_reader/extensions/string.dart';
  ...
  _setupMediaEvents() async {
    ...
    for (var event in widget._chapter.smil.events) {
      if (event.hasAudio && widget._shouldPlayAudio) {
        ...
        final audioBegin = event.audioBeginTimestamp.iso8601ExtendedFormatToMilliseconds();
        final audioDuration = event.audioEndTimestamp.iso8601ExtendedFormatToMilliseconds();
      ...
      if (event.hasTextHighlight) {
        ...
        final textHighlightBegin = event.hasAudio ? event.audioBeginTimestamp.iso8601ExtendedFormatToMilliseconds() : 0;
        final textHighlightOperation = Timer(
          Duration(milliseconds: textHighlightBegin), () async => {
            _highlight(event.highlightedLabelId)
          }
        );
